...
train_loader = torch.utils.data.DataLoader(train1, batch_size=400,shuffle=True, num_workers=0)
val_loader = torch.utils.data.DataLoader(val1, batch_size=400,shuffle=True, num_workers=0)

# iterate over hyperparameters of choice, at the moment just learning rate
...
for lr in lr_values:
        ...
        plt.xlabel('epoch')
        plt.ylabel('accuracy')
        plt.savefig('figures/trainset20000.png', bbox_inches='tight', dpi=200)
        plt.show()
...
pickle.dump(acc_history, open('figures/trainset20000'+'.p','wb') )
#Save the model
torch.save(best_model.state_dict(), "models/final_fixed_cnn1.pth.tar")
...
for lr in lr_values:
        k += 1
        print("##### ROUND "+str(k)+" #####")
              #load model with 3 fully connected layers at the end
        model_ft = models.densenet161(pretrained=True)
        for param in model_ft.features.parameters():
                    param.requires_grad = False
        num_ftrs = model_ft.classifier.in_features
        ...
        plt.xlabel('epoch')
        plt.ylabel('accuracy')
        plt.savefig('figures/trainset20000_3layer.png', bbox_inches='tight', dpi=200)
        plt.show()
...
acc_history = {'train_loss': solver.train_loss_history,
               ...
               'val_loss': solver.val_loss_history,
               'val_acc': solver.val_acc_history}
pickle.dump(acc_history, open('figures/trainset20000_3layer'+'.p','wb') )
#Save the model
torch.save(best_model.state_dict(), "models/final_fixed_cnn_3layer.pth.tar")
print("\n==========\nModel successfully saved!\n==========")