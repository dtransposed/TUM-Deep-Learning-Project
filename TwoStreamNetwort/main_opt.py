...
#Precomputed path to training and val set
train_paths,val_paths = pickle.load(open('train_test_paths.p','rb'))

print('Training set consists of ' + str(len(train_paths)) + ' videos.')
print('Validation set consists of ' + str(len(val_paths)) + ' videos.')

train_loader = VDL.load_videos_path(train_paths,batchsize=10)
...
pred_scores_history, pred_scores_history_train, loss_history, best_perf, best_ep = solver.train(models, train_loader, val_loader, N, num_epochs=8, 
                                                                                                train_CNN=False, train_rgb=False, train_opt=True, pure_train=True)
#Dump results
pickle.dump([pred_scores_history, pred_scores_history_train, loss_history],open('4500samples.p','wb'))
